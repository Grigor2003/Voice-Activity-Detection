...
class DGCGCGD_13_7_Accent(nn.Module):
    def __init__(self, input_dim):
        super().__init__()
        self.fc1 = nn.Linear(input_dim, 128)

        self.gru1 = nn.GRU(128, 64, 1, batch_first=True)
        self.conv2d1 = nn.Conv2d(1, 4, kernel_size=(1, 13), padding='same')
        self.gru2 = nn.GRU(64, 32, 1, batch_first=True)
        self.conv2d2 = nn.Conv2d(4, 1, kernel_size=(1, 7), padding='same')
    ...
    def forward(self, x: torch.Tensor, padding_mask=None, hidden_state=None):
        ...
        last_frame = hiddens3[:, -1]
        out = self.fc2(last_frame)
